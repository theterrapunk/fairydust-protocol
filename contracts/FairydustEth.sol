...
contract FairydustEth is ReentrancyGuard {
  ...
  function convert(uint256 _amount, bool asEth) external payable nonReentrant noContractAllowed expectInitialized {
    require(!pauseConvert, "Fairydust: conversions are paused.");
    if (asEth) {
      require(_amount == msg.value, "_amount != msg.value");
      IWETH9(weth).deposit{value: _amount}();
    } else {  
    ...
    require(xtoken.hasMinted(address(this)) <= xtoken.ceiling(address(this)),"fETH: Fairydust's ceiling was breached.");
  ...
  ///
  modifier onlyKeeper() {
    require(keepers[msg.sender], "Fairydust: only keepers.");
    _;
  }
