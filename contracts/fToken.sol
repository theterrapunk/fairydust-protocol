...
contract FToken is AccessControl, ERC20("Fairy Dust CAKE", "fCAKE") {
  ...
  event Paused(address fairydustAddress, bool isPaused);
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "fCAKE: Fairydust is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "fCAKE: Fairydust is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"fCAKE: Fairydust's ceiling was breached.");
    require(!paused[msg.sender], "fCAKE: user is currently paused.");
